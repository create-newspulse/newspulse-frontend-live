...
import { submitCommunityStory, type CommunityReporterAgeGroup, type SubmitCommunityStoryResult } from '../../src/lib/communityReporterApi';
...
const AGE_GROUP_OPTIONS: Array<{ value: CommunityReporterAgeGroup; label: string }> = [
...
export default function SubmissionExperience({
  ...
  const handleSubmitStep2: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      ...
      const result = await submitCommunityStory({
        reporterAccountId: session?.email || undefined,
      ...
      if (!result.ok) {
        setSubmitError(result.message || 'Please check the story details and try submitting again.');
        setSubmitStatus('error');
        return;
      }

      const storyId = result.referenceId || '';
      const reporterEmail = signUpData.email?.trim();
      try {
      ...
      setSubmitError("We couldn't submit your story right now. Please try again.");
      setSubmitStatus('error');
    } finally {
  ...
  const submissionLinkedToReporter = Boolean(session?.email || isPortalView);
  const successTarget = isPortalView ? '/reporter/dashboard' : portalTrackHref;
  ...
  return (
    <div className="min-h-screen bg-white text-black dark:bg-dark-primary dark:text-dark-text">
      ...
      <section className={`relative bg-gradient-to-br from-blue-50 to-indigo-50 px-4 ${isPortalView ? 'py-10' : 'py-16'} dark:from-gray-800 dark:to-gray-900`}>
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mb-4 rounded-xl border border-gray-200 bg-white p-6 shadow-md dark:border-gray-700 dark:bg-gray-800">
            ...
            <Link href="/community-reporter/guidelines" className="mt-4 inline-flex text-sm font-semibold text-blue-700 hover:underline">
      ...
      <section id="submit-form" className="px-4 py-10">
<div className="mx-auto max-w-3xl">
<form onSubmit={step === 1 ? undefined : handleSubmitStep2} className="space-y-6 rounded-2xl border border-gray-200 bg-white p-8 shadow-lg dark:border-gray-700 dark:bg-gray-800">
<h2 className="mb-2 text-2xl font-bold">{step === 1 ? 'Reporter Details' : 'Submit Your Story'}</h2>
<div className="rounded-lg border border-dashed border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">Shared submission flow: both the public page and Reporter Portal send stories into the same existing community reporter system.</div>{step === 1 ? (<div className="space-y-6 border-b border-gray-200 pb-6 dark:border-gray-700">
<div>{(savedDetailsRestoredFromStorage || hasSavedDetailsInStorage) ? <div className="rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">
<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
<div>
<p className="font-semibold">Using saved reporter details</p>
<p className="mt-0.5 text-xs">These details were restored from this browser.</p>
</div>
<div className="flex flex-wrap gap-2">
<button type="button" className="rounded-lg border border-blue-300 bg-white px-3 py-1.5 text-xs font-semibold text-blue-800" onClick={() => clearSavedReporterDetails({ startFresh: false })}>Clear saved details</button>
<button type="button" className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700" onClick={() => clearSavedReporterDetails({ startFresh: true })}>Start fresh</button>
</div>
</div>
</div> : null}</div>
<div>
<label htmlFor="fullName" className="mb-1 block font-medium">Full name *</label>
...
<input id="email" type="email" value={signUpData.email} onChange={(e) => setSignUpData((current) => ({ ...current, email: e.target.value }))} disabled={readonlyMode || Boolean(isPortalView && initialEmail)} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />{errors.email ? <p className="mt-1 text-xs text-red-600">{errors.email}</p> : null}</div>
<div>
<label htmlFor="phone" className="mb-1 block font-medium">Phone (recommended)</label>
<input id="phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" value={signUpData.phone} onChange={(e) => setSignUpData((current) => ({ ...current, phone: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />{errors.phone ? <p className="mt-1 text-xs text-red-600">{errors.phone}</p> : null}<p className="mt-1 text-xs text-gray-500">Recommended for editor follow-up and directory enrichment. Never shown publicly.</p>
</div>
<div>
<label htmlFor="whatsapp" className="mb-1 block font-medium">WhatsApp (optional)</label>
<input id="whatsapp" name="whatsapp" type="tel" inputMode="tel" autoComplete="tel-national" value={signUpData.whatsapp} onChange={(e) => setSignUpData((current) => ({ ...current, whatsapp: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />{errors.whatsapp ? <p className="mt-1 text-xs text-red-600">{errors.whatsapp}</p> : null}</div>
<div className="grid gap-4 md:grid-cols-2">
<div>
<label htmlFor="city" className="mb-1 block font-medium">City</label>
<input id="city" type="text" value={signUpData.city} onChange={(e) => setSignUpData((current) => ({ ...current, city: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />
</div>
<div>
<label htmlFor="district" className="mb-1 block font-medium">District</label>
<input id="district" type="text" value={signUpData.district} onChange={(e) => setSignUpData((current) => ({ ...current, district: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />
</div>
<div>
<label htmlFor="state" className="mb-1 block font-medium">State</label>
<input id="state" type="text" value={signUpData.state} onChange={(e) => setSignUpData((current) => ({ ...current, state: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />
</div>
<div>
<label htmlFor="country" className="mb-1 block font-medium">Country</label>
<input id="country" type="text" value={signUpData.country} onChange={(e) => setSignUpData((current) => ({ ...current, country: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />
</div>
</div>
<div>
<label className="mb-2 block font-medium">Preferred languages</label>
<div className="flex flex-wrap gap-2">{LANG_OPTIONS.map((code) => <button type="button" key={code} onClick={() => setSignUpData((current) => ({ ...current, preferredLanguages: current.preferredLanguages.includes(code) ? current.preferredLanguages.filter((item) => item !== code) : [...current.preferredLanguages, code] }))} disabled={readonlyMode} className={`rounded-full border px-3 py-1 ${signUpData.preferredLanguages.includes(code) ? 'border-purple-600 bg-purple-600 text-white' : 'border-gray-300 bg-white text-gray-800 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200'}`}>{code}</button>)}</div>
</div>
<div>
<label htmlFor="heardAbout" className="mb-1 block font-medium">How did you hear about News Pulse? (optional)</label>
<input id="heardAbout" type="text" value={signUpData.heardAbout || ''} onChange={(e) => setSignUpData((current) => ({ ...current, heardAbout: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />
</div>
<div>
<label className="mb-2 block font-medium">I am</label>
<div className="flex flex-col gap-3 md:flex-row">
<label className="inline-flex items-center gap-2">
<input type="radio" name="reporterType" checked={reporterType === 'community'} onChange={() => { setReporterType('community'); setSignUpData((current) => ({ ...current, isProfessionalJournalist: false })); }} disabled={readonlyMode} />
<span>I am a Community Reporter</span>
</label>{journalistApplicationsOpen ? <label className="inline-flex items-center gap-2">
<input type="radio" name="reporterType" checked={reporterType === 'journalist'} onChange={() => { setReporterType('journalist'); setSignUpData((current) => ({ ...current, isProfessionalJournalist: true })); }} disabled={readonlyMode} />
<span>I am a Professional Journalist / Media Person</span>
</label> : <p className="text-xs text-gray-600 dark:text-gray-300">Journalist verification applications are currently closed.</p>}</div>
{reporterType === 'journalist' ? <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950">
Professional journalists may submit information here, but professional verification, onboarding, credentials and formal News Pulse journalist workflows are handled separately through the Journalist Desk. <Link href="/journalist-desk" className="font-semibold text-amber-950 underline">Visit Journalist Desk</Link>
</div> : null}
</div>{reporterType === 'community' ? <div>
<label className="mb-2 block font-medium">Beats / interests</label>
<div className="flex flex-wrap gap-2">{COMMUNITY_INTERESTS.map((item) => <button type="button" key={item} onClick={() => setSignUpData((current) => ({ ...current, communityInterests: current.communityInterests.includes(item) ? current.communityInterests.filter((value) => value !== item) : [...current.communityInterests, item] }))} disabled={readonlyMode} className={`rounded-full border px-3 py-1 ${signUpData.communityInterests.includes(item) ? 'border-blue-600 bg-blue-600 text-white' : 'border-gray-300 bg-white text-gray-800 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200'}`}>{item}</button>)}</div>
</div> : null}<div className="space-y-3">
<div className="flex items-start gap-3">
<input id="consentToContact" type="checkbox" checked={signUpData.consentToContact} onChange={(e) => setSignUpData((current) => ({ ...current, consentToContact: e.target.checked }))} disabled={readonlyMode} className="mt-1 h-5 w-5 rounded border-gray-300 focus:ring-blue-500" />
...
<div className="flex items-start gap-3">
<input id="ethics" type="checkbox" checked={signUpData.generalEthicsAccepted} onChange={(e) => setSignUpData((current) => ({ ...current, generalEthicsAccepted: e.target.checked }))} disabled={readonlyMode} className="mt-1 h-5 w-5 rounded border-gray-300 focus:ring-blue-500" />
...
</div>{missingRecommendedProfileFields.length ? <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
...
</div> : null}<div className="pt-2">
<button type="button" onClick={() => { if (validateStep1()) setStep(2); }} disabled={readonlyMode} className={`w-full rounded-lg px-6 py-3 font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 ${readonlyMode ? 'cursor-not-allowed bg-gray-400 text-white' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>Next</button>
</div>
</div>) : (<>
<div>
<label htmlFor="category" className="mb-1 block font-medium">Category *</label>
<select id="category" value={story.category} onChange={(e) => setStory((current) => ({ ...current, category: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700">
<option value="">{COMMUNITY_REPORTER_CATEGORY_PLACEHOLDER}</option>{COMMUNITY_REPORTER_CATEGORY_OPTIONS.map((category) => <option key={category.value} value={category.value}>{category.label}</option>)}</select>
</div>
<div>
<label className="mb-2 block font-medium">Coverage scope *</label>
<div className="flex flex-wrap items-center gap-4">{['regional', 'national', 'international'].map((scope) => <label key={scope} className="inline-flex items-center gap-2">
<input type="radio" name="coverageScope" checked={story.coverageScope === scope} onChange={() => setStory((current) => ({ ...current, coverageScope: scope as StoryFormState['coverageScope'] }))} disabled={readonlyMode} />
<span>{scope.charAt(0).toUpperCase() + scope.slice(1)}</span>
</label>)}</div>{errors.coverageScope ? <p className="mt-1 text-xs text-red-600">{errors.coverageScope}</p> : null}</div>
<div>
<label htmlFor="headline" className="mb-1 block font-medium">Headline *</label>
<input id="headline" type="text" maxLength={150} value={story.headline} onChange={(e) => setStory((current) => ({ ...current, headline: e.target.value }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700" />{errors.headline ? <p className="mt-1 text-xs text-red-600">{errors.headline}</p> : null}</div>
<div>
<label htmlFor="storyBody" className="mb-1 block font-medium">Story *</label>
...
<div>
<label htmlFor="ageGroup" className="mb-1 block font-medium">Age group *</label>
<select id="ageGroup" value={story.ageGroup} onChange={(e) => setStory((current) => ({ ...current, ageGroup: e.target.value as StoryFormState['ageGroup'] }))} disabled={readonlyMode} className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 dark:border-gray-600 dark:bg-gray-700">
<option value="">Select age group</option>
{AGE_GROUP_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
</select>{errors.ageGroup ? <p className="mt-1 text-xs text-red-600">{errors.ageGroup}</p> : null}</div>{submitStatus === 'error' ? <div className="mt-4 rounded-lg border border-red-300 bg-red-50 p-4 text-red-700">
<p className="font-semibold">We couldn't submit your story right now. Please try again in a few minutes.</p>{submitError ? <p className="mt-1 text-xs">{submitError}</p> : null}</div> : null}{submitStatus === 'success' && submitResult ? <div className="mt-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-900">
<p className="font-semibold">Thank you. Your story has been submitted to News Pulse for review.</p>{submitResult.referenceId ? <p className="mt-1">Your reference ID: {submitResult.referenceId}</p> : null}<p className="mt-1">Status: Under review. Usually reviewed within 24–48 hours.</p>
<div className="mt-3 flex flex-wrap gap-3">
{submissionLinkedToReporter ? <button type="button" className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700" onClick={() => router.push(successTarget)}>{isPortalView ? 'View my dashboard' : 'Track your submissions'}</button> : <Link href={portalLoginHref} className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700">Want to track future submissions? Sign in to the Reporter Portal.</Link>}
<button type="button" className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-800" onClick={() => { setStory(initialStory); setSubmitStatus('idle'); setSubmitResult(null); setSubmitError(null); }}>Submit another story</button>
</div>
</div> : null}<div className="mt-4 flex items-center gap-3">
<button type="button" onClick={() => setStep(1)} className="rounded-lg border border-gray-300 px-4 py-2 dark:border-gray-600">Back</button>
<button type="submit" disabled={readonlyMode || isSubmitting} className={`rounded-lg px-6 py-2 font-semibold ${(readonlyMode || isSubmitting) ? 'cursor-not-allowed bg-gray-400' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>{isSubmitting ? 'Submitting…' : 'Submit Story'}</button>
</div>
</>)}</form>
</div>
</section>
