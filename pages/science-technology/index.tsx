...
export const getServerSideProps: GetServerSideProps<Props> = async (ctx) => {
  ...
  const { items, error } = await fetchCategoryNews({ categoryKey: 'science-technology', limit: 30 });

  return {
    props: {
      categoryKey: 'science-technology',
...
export default function ScienceTechnologyPage({ categoryKey, title, articles, error }: Props) {
  return (
    <CategoryFeedPage
