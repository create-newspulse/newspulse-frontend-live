...
const TrendingStories = ({ articles }) => {
  ...
  return (
    <section className="mt-8">
      <h2 className="text-xl font-bold mb-3 text-newsPulse-red">🔥 {t('trending.title')}</h2>
        ...
        {articles.slice(0, 8).map((article) => {
          ...
          return (
            <div
              ...
              className="group w-[220px] shrink-0 bg-newsPulse-white rounded-xl shadow-md snap-start sm:w-[240px]"
            >
              <StoryImage
                storyId={storyId}
                src={coverImageUrl}
              ...
              />
              <div className="p-2">
                <p className="text-sm font-medium line-clamp-2 text-newsPulse-navy">{article.title}</p>
              </div>
            </div>
          );
        })}
