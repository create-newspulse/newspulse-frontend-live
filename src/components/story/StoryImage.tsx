...
export function StoryImage({
  ...
  const isInteractivePhoto = resolvedCoverFitMode === 'photo';
  ...
  const foregroundImageClassName = isGraphicDetailHero
    ...
    : 'absolute inset-0 h-full w-full object-cover';
  const imageMotionClassName = !isInteractivePhoto
    ? 'transition-opacity duration-300 ease-out'
    : isDetailHero
      ? 'transform-gpu transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.035]'
      : 'transform-gpu transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.045]';
  const pressedImageClassName = isPressed && isInteractivePhoto
    ? isDetailHero
      ? 'scale-[1.018]'
      : 'scale-[1.028]'
    : null;
      ...
      {showImage ? (
        <>
          ...
          <img
            ...
            className={cx(foregroundImageClassName, imageMotionClassName, pressedImageClassName)}
            onLoad={(event) => {
