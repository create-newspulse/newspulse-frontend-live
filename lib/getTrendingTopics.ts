import { DEFAULT_TRENDING_TOPICS, type TrendingTopic } from './config/trendingTopics';
...
const HREF_BY_TOPIC_KEY: Record<string, string> = {
  breaking: '/breaking',
  sports: '/sports',
  weather: '/topic/weather',
  'gold-rates': '/topic/gold-rates',
  'fuel-prices': '/topic/fuel-prices',
  markets: '/topic/markets',
  'tech-ai': '/topic/tech-ai',
...
function normalizeTopic(raw: unknown): TrendingTopic | null {
  ...
  const mappedHref = HREF_BY_TOPIC_KEY[key.toLowerCase()];

  // If backend doesn't provide a valid href (or provides a known-bad one like /india),
  // route to a stable in-app destination.
  const href = mappedHref
    ? mappedHref
    : rawHref && rawHref.startsWith('/') && rawHref !== '/india'
      ? rawHref
      : toSearchHref(label);
