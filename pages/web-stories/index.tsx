...
export const getServerSideProps: GetServerSideProps<Props> = async (ctx) => {
  ...
  const { items, error } = await fetchCategoryNews({ categoryKey: 'web-stories', limit: 30 });

  return {
    props: {
      categoryKey: 'web-stories',
      title: 'Web Stories',
      articles: items,
      error: error ?? null,
      messages,
...
export default function WebStoriesPage({ categoryKey, title, articles, error }: Props) {
  return (
    <CategoryFeedPage
