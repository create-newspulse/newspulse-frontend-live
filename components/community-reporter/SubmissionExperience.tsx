...
export default function SubmissionExperience({
  ...
  const missingRecommendedProfileFields = useMemo(() => {
    ...
    if (!signUpData.phone.trim() && !signUpData.whatsapp.trim()) missing.push('phone/WhatsApp');
  ...
  const validateStep1 = () => {
    ...
    if (hasEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(signUpData.email)) nextErrors.email = 'Enter a valid email.';
    if (!hasEmail && !hasPhone && !hasWhatsApp) nextErrors.email = 'Add at least one contact method (email, phone, or WhatsApp).';
